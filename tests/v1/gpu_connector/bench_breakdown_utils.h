...
static inline uint64_t ts_diff_ns(const struct timespec& a,
...
static inline void report_results(const char* label,
  ...
  double avg_lat_us =
      total_ops > 0 ? (double)total_io_time / (total_ops * 1000.0) : 0;
  double cpu_util =
      total_wall_ns > 0 ? (double)total_cpu_ns / total_wall_ns : 0;
  double cpu_us_per_io =
      total_ops > 0 ? (double)total_cpu_ns / (total_ops * 1000.0) : 0;
...
static inline size_t parse_size(const char* str) {
  ...
  if (unit == 'K' || unit == 'k')
    ...
    return (size_t)strtoull(str, nullptr, 10);
  s.pop_back();
...
static inline bool parse_bench_opts(int argc, char** argv, BenchOpts& opts) {
  static struct option long_options[] = {
      ...
      {nullptr, 0, nullptr, 0}};
  int opt;
  while ((opt = getopt_long(argc, argv, "f:l:s:d:t:i:m:jh", long_options,
                            nullptr)) != -1) {
    switch (opt) {
      case 'f':
        ...
        break;
      case 'l':
        opts.length = parse_size(optarg);
        break;
      case 's':
        opts.io_size = parse_size(optarg);
        break;
      case 'd':
        opts.gpu_id = atoi(optarg);
        break;
      case 't':
        opts.num_threads = atoi(optarg);
        break;
      case 'i':
        opts.io_depth = atoi(optarg);
        break;
      case 'm':
        if (strcmp(optarg, "write") == 0) {
          opts.is_write = true;
        } else if (strcmp(optarg, "batch-read") == 0) {
          opts.is_batch = true;
          opts.is_write = false;
        } else if (strcmp(optarg, "batch-write") == 0) {
          opts.is_batch = true;
          opts.is_write = true;
        } else if (strcmp(optarg, "async-read") == 0) {
          opts.is_async = true;
          opts.is_write = false;
        } else if (strcmp(optarg, "async-write") == 0) {
          opts.is_async = true;
          opts.is_write = true;
        }
        break;
      ...
      default:
        fprintf(stderr,
                "Usage: %s -f <file> [-l length] [-s io_size] [-d gpu] [-t "
                "threads] [-i depth] [-m read|write] [-j]\n",
                argv[0]);
        return false;
    }
  }
