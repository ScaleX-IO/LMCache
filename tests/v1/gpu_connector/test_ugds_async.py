...
def _ugds_available() -> bool:
    """Whether uGDS hardware is present (ugds_drv loaded + libugds.so loadable)."""
    if not torch.cuda.is_available():
        return False
    if not any(os.path.exists(f"/dev/ugds_drv{i}") for i in range(4)):
        return False
    try:
...
class TestUgdsRoundtrip:
    ...
    def _find_device(self) -> str:
        for i in range(4):
            path = f"/dev/ugds_drv{i}"
            if os.path.exists(path):
                return path
        raise RuntimeError("no ugds_drv device found")
